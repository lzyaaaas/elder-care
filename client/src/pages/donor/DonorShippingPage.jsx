...
export function DonorShippingPage() {
  ...
  useEffect(() => {
    ...
    async function loadItems() {
      try {
        const result = await apiRequest("/donor-portal/shipping", { authMode: "donor" });
  ...
  return (
    <section className="rounded-[2rem] border border-white/60 bg-white/85 p-6 shadow-soft">
      <p className="text-sm font-semibold uppercase tracking-[0.22em] text-pine-800">Shipping</p>
      <h2 className="mt-4 text-2xl font-semibold text-ink-950">Gift kit delivery updates</h2>
      ...
      <div className="mt-8 overflow-x-auto">
        ...
        {!items.length && !error ? <p className="py-8 text-sm text-ink-900/55">No shipping records yet.</p> : null}
      </div>
    </section>
  );
}
